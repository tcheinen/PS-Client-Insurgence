...
const Tools = {
	...
	createReplayFile(room: any) {
		...
		buf += '</div>\n';
		buf += '<script>\n';
		buf += 'let daily = Math.floor(Date.now()/1000/60/60/24);document.write(\'<script src="http://storage.googleapis.com/ps-resources-insurgence/psclient/js/replay-embed.js?version\'+daily+\'"></\'+\'script>\');\n';
		buf += '</script>\n';
		return buf;
	},
